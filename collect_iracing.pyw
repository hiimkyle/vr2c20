...
def hec_send(ir_json):
    event = {}
    #event['time'] = str(datetime.utcnow())
    #event['host'] = racer_name
    event['event'] = ir_json
    url = 'http://splunk.bigdatabeard.com:8088/services/collector' 
    header = {'Authorization' : '{}'.format('Splunk ' + '88a0de38-beb0-4863-8c79-393cd8d23ab6')}
...
def loop():
    for key, value in mydict.items():
        ...
        mydict.update({key: value})
    #data_logger.info(json.dumps(vehicle_dict))
    app_logger.info(time.ctime() + " mydict: logged")
    hec_send(mydict)
    #time.sleep(vehicle_sleep)
    ...
    try:
        ...
        while True:
            ...
            if state.ir_connected:
                 loop()
                # TW if i modulo seconds // if i > 30, camera loop i = 0 
                # camera_loop()
                # compute_loop()
                # environment_loop()
                # race_loop()
